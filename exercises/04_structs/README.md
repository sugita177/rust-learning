...
複数の関連するデータを意味のある1つの型としてまとめる「構造体（`struct`）」の基本、フィールド初期化省略記法、構造体更新記法（`..`）と所有権の部分ムーブ、デバッグ出力（`#[derive(Debug)]`）、タプル構造体、および「メソッド構文（`impl`）」と関連関数について学ぶ演習です。
...
### 1.6 メソッド構文と `impl` ブロック
構造体の文脈においてデータと振る舞いを紐づける関数を「メソッド」と呼び、**`impl 構造体名 { ... }`** ブロック内に定義します。
...
impl Rectangle {
    ...
    // 関連関数（第1引数に self を取らない、コンストラクタ的な関数）
...
#### 第1引数 `self` の3つのバリエーション
| 宣言 | 意味 | 用途 |
| :--- | :--- | :--- |
| **`&self`** | インスタンスの不変借用 | データを読み取るだけ（最も一般的） |
| **`&mut self`** | インスタンスの可変借用 | フィールドの値を書き換える |
| **`self`** | インスタンスの所有権ムーブ | インスタンスを消費し、元を消滅させたい時（別の型への変換など） |

#### 自動参照・自動参照解除（Automatic Referencing）
C/C++ ではポインタと実体で `object.method()` と `pointer->method()` を使い分ける必要がありましたが、Rust では `rect.area()` と書くだけで、メソッド定義の `&self` / `&mut self` に応じてコンパイラが自動的に `(&rect).area()` や `(&mut rect).scale(2)` と解釈してくれます。

#### 関連関数（Associated Functions）と `Self` キーワード
- 第1引数に `self` を持たない関数は、型そのものに属する関数（クラスメソッド相当）として扱われます。
- 呼び出しは `Rectangle::square(25)` のように `::` を使用します。
- `impl` ブロック内では、自分自身の型名を指すエイリアスとして **`Self`**（大文字始まり）が使えます。
...
### 2.4 [メソッド検証] メソッドと関連関数の実証
- **検証コード**:
  ```rust
  let rect1 = Rectangle { width: 30, height: 50 };
  let rect2 = Rectangle { width: 10, height: 40 };
  let sq = Rectangle::square(25); // 関連関数

  println!("area: {}", rect1.area()); // &self
  println!("can_hold: {}", rect1.can_hold(&rect2)); // 別の借用を受け取る

  // 不変変数 rect1 を可変変数として再束縛（シャドーイング）
  let mut rect1 = rect1;
  rect1.scale(2); // &mut self
  println!("after scale area: {}", rect1.area());
  ```
- **実証できたこと**:
  - `&self` による読み取り専用メソッド（`area`）、複数の参照を受け取るメソッド（`can_hold`）が直感的に呼び出せること。
  - `&mut self` を呼ぶにはインスタンスが `mut` である必要があり、不変インスタンスから `let mut rect1 = rect1;` とシャドーイングすることで安全に可変へ移行できること。
  - 関連関数 `Rectangle::square` により、名前空間を構造体に閉じ込めたコンストラクタが作成できること。
